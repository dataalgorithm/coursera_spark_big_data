...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val strFields = StructField(columnNames.head, StringType, false) +: columnNames.tail.map(StructField(_, DoubleType, false))
    StructType(strFields)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsAct = columnNames
      .filter(c => isColumnInActivityType(c, Set("t01", "t03", "t11", "t1801", "t1803")))
      .map(col(_))
    val workAct = columnNames
      .filter(c => isColumnInActivityType(c, Set("t05", "t1805"))).map(col(_))
    val otherAct = columnNames
      .filter(c => isColumnInActivityType(
        c,
        Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18"),
        Set("t1801", "t1803", "t1805")))
      .map(col(_))
    (primaryNeedsAct, workAct, otherAct)
  }

  private def isColumnInActivityType(colName: String, allowedSet: Set[String], excludedSet: Set[String] = Set.empty[String]): Boolean = {
    val isAllowed: Boolean = !allowedSet.filter(colName.startsWith(_)).isEmpty
    val isExcluded: Boolean = !excludedSet.filter(colName.startsWith(_)).isEmpty
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map{case ((working, sex, age),primA, workA, otherA) =>
        TimeUsageRow(working, sex, age, primA, workA, otherA)}
      .orderBy("working", "sex", "age")
